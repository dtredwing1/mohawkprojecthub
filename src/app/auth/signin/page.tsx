...
import { Shield, Sparkles, ArrowRight, Lock, AlertTriangle } from 'lucide-react';
...
function SignInContent() {
  ...
  const getErrorMessage = (errCode: string | null) => {
    ...
    switch (errCode) {
      case 'OAuthCallback':
        return 'Google authentication could not be completed. Please ensure your Google account is added as a Test User in the Google Cloud OAuth Consent Screen, and that the client secret has been updated and deployed.';
      case 'AccessDenied':
        return 'Access restricted. Your Google account is not on the collaborator allowlist. Ask an administrator to invite your email.';
      case 'Configuration':
        return 'There is a problem with the server authentication configuration.';
      default:
        return `Authentication failed (${errCode}). Please try again.`;
    }
  };
  ...
  return (
    <div className="w-full max-w-md bg-slate-900/80 border border-slate-800 rounded-2xl p-8 shadow-2xl backdrop-blur-md relative z-10 space-y-6">
      ...
      {/* Error Alert if any */}
      {errorMessage && (
        <div className="p-3.5 rounded-xl bg-red-950/60 border border-red-500/40 text-xs text-red-200 space-y-1.5 animate-in fade-in">
          <div className="flex items-center gap-2 font-semibold text-red-400">
            <AlertTriangle className="w-4 h-4 text-red-400 shrink-0" />
            <span>Sign-in Notice ({error})</span>
          </div>
